function settingsApp() {
  return {
    ...
    async resetChrome() {
      if (!confirm('Reset Chrome browser? This will clear lock files and restart the browser. Use this if Chrome is stuck or showing a black screen.')) {
        return;
      }
      this.resettingChrome = true;
      try {
        const res = await fetch('/api/chrome/reset', { method: 'POST' });
        if (res.ok) {
          const data = await res.json();
          this.showToast('Chrome reset successfully. Browser is restarting...', 'success');
          // Poll for browser to come back up
          setTimeout(() => this.loadStatus(), 3000);
          setTimeout(() => this.loadStatus(), 8000);
          setTimeout(() => this.loadStatus(), 15000);
        } else {
          const err = await res.json();
          this.showToast(err.error || 'Failed to reset Chrome', 'error');
        }
      } catch (err) {
        console.error('Failed to reset Chrome:', err);
        this.showToast('Failed to reset Chrome', 'error');
      } finally {
        this.resettingChrome = false;
      }
    },
